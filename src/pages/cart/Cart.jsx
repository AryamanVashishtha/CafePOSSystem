...
function Cart() {
  ...
  const buyNow = async () => {
    ...
    var options = {
      key: "rzp_test_LWHOa6RA1jVpOB",
      key_secret: "Ugh4idfOiHAzfR0ft0bcyroA",
      amount: parseInt(grandTotal * 100),
      currency: "INR",
      order_receipt: 'order_rcptid_' + name,
      name: "JIIT Cafeteria",
      inStock: "for testing purpose",
      handler: function (response) {
        ...
        sendOrderConfirmation(orderInfo);
        try {
        ...
        <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0 ">
          <div className="rounded-lg md:w-2/3 ">
            {cartItems.map((item, index) => {
              const { title, price, imageUrl, uid } = item; // Assuming `uid` is a unique identifier
              return (
                <div
                  key={uid || index} // Add the key prop here, use `uid` if available, otherwise use `index`
          ...
          <div className="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3" style={{ backgroundColor: mode === 'dark' ? 'rgb(32 33 34)' : '', color: mode === 'dark' ? 'white' : '', }}>
            ...
            <div className="flex justify-between">
              <p className="text-gray-700" style={{ color: mode === 'dark' ? 'white' : '' }}>Taxes(incl. in total)
              </p>
